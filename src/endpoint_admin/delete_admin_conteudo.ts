import { Function_deleteContentFileFromR2, Function_deleteD1, Function_getAdminAuthenticated, Function_getD1, Function_getFuncionName, Function_getResponseError, Function_isError } from "../function_global"


type Type_DeleteAdminConteudoResponse = {
	success: true;
}


export class Class_DeleteAdminConteudo {
	public static async main(Parameter_request: Request, Parameter_env: Env, Parameter_context: ExecutionContext): Promise<Response> {
		try {
			...
			const Const_contentArray = await Function_getD1(Parameter_env, 'content', 1, 1, ['content_uuid', 'preview_file_uuid_content', 'full_file_uuid_content'], {
			...
			if (Function_isError(Const_contentDeleted)) {
				return Function_getResponseError(Const_contentDeleted, 455, 'Error deleting admin content')
			}
			// /\ Deleta conteudo do D1
			...
			const Const_responseBody: Type_DeleteAdminConteudoResponse = {
				success: true
			}
			...
			return Function_getResponseError({ typ: 'catch', msg: 'Unhandled error deleting admin content', inf: { Parameter_error, Parameter_context, Parameter_request, Parameter_env }, loc: Function_getFuncionName(), err: true }, 499, 'Unhandled endpoint error')
		}
	}
}
