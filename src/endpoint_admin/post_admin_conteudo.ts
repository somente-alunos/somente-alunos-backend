...
function Function_getNullableStringOrNullForEmpty(Parameter_value: unknown): string | null | undefined {
	if (Parameter_value === null) {
		return null
	}
	...
	const Const_value = Parameter_value.trim()
	if (Const_value.length <= 0 || Const_value.toLowerCase() === 'null') {
		return null
	}
	...
	public static async main(Parameter_request: Request, Parameter_env: Env, Parameter_context: ExecutionContext): Promise<Response> {
		try {
			...
			const Const_body: Partial<Type_PostAdminConteudoBody> = {
				...
				prevision_content: Function_getNullableStringOrNullForEmpty(Const_formData.get('prevision_content')),
			...
			const Const_hasPrevisionContentValue = typeof Const_body.prevision_content === 'string'
			if (Const_hasPrevisionContentValue) {
				Const_body.preview_file_uuid_content = null
				Const_body.full_file_uuid_content = null
				Const_body.preview_file_content = undefined
				Const_body.full_file_content = undefined
			}

			if (!Const_hasPrevisionContentValue && Const_formData.has('preview_file_uuid_content') && Const_body.preview_file_uuid_content === undefined) {
			...
			if (!Const_hasPrevisionContentValue && Const_formData.has('full_file_uuid_content') && Const_body.full_file_uuid_content === undefined) {
			...
			if (!Const_hasPrevisionContentValue && Const_body.preview_file_content instanceof File && Const_formData.has('preview_file_uuid_content')) {
			...
			if (!Const_hasPrevisionContentValue && Const_body.full_file_content instanceof File && Const_formData.has('full_file_uuid_content')) {
			...
			if (Let_previewFileValidated) {
				...
				console.log(`[CPF] ${Const_body.class_content} Const_previewUploadResult`, Const_previewUploadResult)
			...
			if (Let_fullFileValidated) {
				...
				console.log(`[CPF] ${Const_body.class_content} Const_fullUploadResult`, Const_fullUploadResult)
