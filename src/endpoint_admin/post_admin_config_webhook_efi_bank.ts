import { Function_configWebhookEfi, Function_generateAcessTokenEfi, Function_generateCreatePaymentPixEfi, Function_getAdminAuthenticated, Function_getFuncionName, Function_getResponseError, Function_getWebhooksEfi, Function_isError } from "../function_global"
	...
	public static async main(Parameter_request: Request, Parameter_env: Env, Parameter_context: ExecutionContext): Promise<Response> {
		try {
			...
			if (Function_isError(Const_generateAcessTokenEfi)) {
				return Function_getResponseError(Const_generateAcessTokenEfi, 452, 'Error generating Efi access token for webhook configuration')
			}
			// /\ Gera token efi

			// \/ Configura webhook na Efi
			const Const_configWebhookEfi = await Function_configWebhookEfi(Parameter_env, Const_generateAcessTokenEfi)
			if (Function_isError(Const_configWebhookEfi)) {
				return Function_getResponseError(Const_configWebhookEfi, 453, 'Error configuring Efi webhook')
			}
			// /\ Configura webhook na Efi
			...
			if (Function_isError(Const_generateCreatePaymentPixEfi)) {
				return Function_getResponseError(Const_generateCreatePaymentPixEfi, 454, 'Error generating Efi PIX charge to test webhook configuration')
			}

			const Const_pixCopiaECola = Const_generateCreatePaymentPixEfi.pixCopiaECola
			if (typeof Const_pixCopiaECola !== 'string') {
				return Function_getResponseError({ typ: 'logical', msg: 'Invalid pixCopiaECola returned from Efi when testing webhook configuration', inf: { pixCopiaECola: Const_pixCopiaECola }, loc: Function_getFuncionName(), err: true }, 455, 'Invalid pixCopiaECola returned from Efi when testing webhook configuration')
			}
			// /\ Testa gerando cobraça de PIX
			...
			if (Function_isError(Const_getWebhooksEfi)) {
				return Function_getResponseError(Const_getWebhooksEfi, 456, 'Error fetching configured webhooks from Efi')
			}
			// /\ Obtem lista de webhooks configurados
			...
			const Const_responseBody: Type_PostAdminConfigWebhookEfiBankResponse = {
				...
				webhooks: Const_getWebhooksEfi.webhooks
			}
