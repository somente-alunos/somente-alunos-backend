import { Function_deleteContentFileFromR2, Function_getD1, Function_getFuncionName, Function_getResponseError, Function_getStudentAuthenticated, Function_getTrimmedStringOrUndefined, Function_getValidatedStudentContentFile, Function_isError, Function_patchD1, Function_postContentFileToR2 } from "../function_global"


type Type_PostStudentConteudoFileBody = {
	content_uuid: string;
	file_role: 'preview' | 'full';
	file: File;
}

type Type_PostStudentConteudoFileResponse = {
	content: Type_objectStudentContentResponse;
	fileRole: Type_PostStudentConteudoFileBody['file_role'];
...
export class Class_PostStudentConteudoFile {
	public static async main(Parameter_request: Request, Parameter_env: Env, Parameter_context: ExecutionContext): Promise<Response> {
		try {
			...
			const Const_body: Partial<Type_PostStudentConteudoFileBody> = {
				content_uuid: Function_getTrimmedStringOrUndefined(Const_formData.get('content_uuid')),
				file_role: Function_getTrimmedStringOrUndefined(Const_formData.get('file_role')) as Type_PostStudentConteudoFileBody['file_role'] | undefined,
			...
			if (Const_content.student_uuid_content !== Const_studentAuthenticated.student_uuid) {
				return Function_getResponseError({ typ: 'logical', msg: 'Student cannot upload file to content from another owner', inf: { Const_contentUuid, contentOwnerUuid: Const_content.student_uuid_content, studentUuid: Const_studentAuthenticated.student_uuid }, loc: Function_getFuncionName(), err: true }, 457, 'Forbidden content file upload')
			}
			// /\ Garante que o conteudo existe e pertence ao aluno autenticado
			...
			if (Function_isError(Const_uploadResult)) {
				return Function_getResponseError(Const_uploadResult, 458, 'Error uploading content file to R2')
			}
			// /\ Faz upload de novo arquivo no R2
			...
			if (Function_isError(Const_contentUpdated)) {
				...
				return Function_getResponseError(Const_contentUpdated, 459, 'Error updating content file UUID on D1')
			}
			// /\ Atualiza UUID do arquivo no D1
			...
			const Const_responseBody: Type_PostStudentConteudoFileResponse = {
			...
			return Function_getResponseError({ typ: 'catch', msg: 'Unhandled error posting student content file', inf: { Parameter_error, Parameter_context, Parameter_request, Parameter_env }, loc: Function_getFuncionName(), err: true }, 499, 'Unhandled endpoint error')
		}
	}
}
