import { Function_getD1, Function_getFuncionName, Function_getResponseError, Function_getStudentAuthenticated, Function_isError, Function_postD1, Function_generateAcessTokenEfi, Function_generateCreatePaymentPixEfi, Function_generatePaymentLinkEfi, Function_generateAcessTokenEfiLinkPayment } from "../function_global"
...
export class Class_PostStudentGerarCobranca {
	public static async main(Parameter_request: Request, Parameter_env: Env, Parameter_context: ExecutionContext): Promise<Response> {
		try {
			...
			// \/ Cria ordered para a cobranca
			const Const_contentUuidArrayBodyString = JSON.stringify([...Const_contentUuidArrayBody].sort())

			// Vê se ja existe uma ordered com o mesmos parametros
			let Let_orderedUuid = ''
			const Const_existingOrdered = await Function_getD1(Parameter_env, 'ordered', 1, 1, ['ordered_uuid'], {
				student_uuid_buyer_ordered: Const_studentAuthenticated.student_uuid,
				content_uuid_array_ordered: Const_contentUuidArrayBodyString,

				total_amount_ordered: Number(Let_totalAmount.toFixed(2)),
				method_ordered: Const_methodBody,

				status_ordered: 'waiting'
			})
			if (Function_isError(Const_existingOrdered)) {
				return Function_getResponseError(Const_existingOrdered, 462, 'Error checking for existing ordered before creating new ordered for PIX charge')
			}

			if (!Const_existingOrdered?.[0]?.ordered_uuid) {
				Let_orderedUuid = crypto.randomUUID()

				const Const_orderedCreated = await Function_postD1(Parameter_env, 'ordered', {
					ordered_uuid: Let_orderedUuid,
				...
				if (Function_isError(Const_orderedCreated)) {
					return Function_getResponseError(Const_orderedCreated, 464, 'Error creating ordered for PIX charge')
				}
			}

			else {
				Let_orderedUuid = Const_existingOrdered[0].ordered_uuid
			}
			// /\ Cria ordered para a cobranca

			// \/ Confere se ja existe no cache
			const Const_cacheKey = `cobranca_v2_${Let_orderedUuid}`
			...
			if (Const_methodBody === 'pix') {
				...
				const Const_orderedUuidWithoutDashe = Let_orderedUuid.replaceAll('-', '')
				const Const_name = `Conteúdo de estudo - Somente Alunos`
				const Const_generateCreatePaymentPixEfi = await Function_generateCreatePaymentPixEfi(Parameter_env, Const_generateAcessTokenEfi, Const_price, Const_orderedUuidWithoutDashe, Const_name)
				...
				const Const_generatePaymentLinkEfi = await Function_generatePaymentLinkEfi(Parameter_env, Const_generateAcessTokenEfi, Const_itemsArray, Let_orderedUuid)
